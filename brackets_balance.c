...
size_t count_strings(char const *const *const string_array, char **result_array,
                     const size_t size);
...
int main() {
  ...
  size_t new_size =
      count_strings((char const *const *const)string_array, result_array, size);
...
size_t count_strings(char const *const *const string_array, char **result_array,
  ...
  for (size_t i = 0; i < size; ++i) {
    if (check_string(string_array[i])) {
      result_array[j] =
          (char *)malloc((strlen(string_array[i]) + 1) * sizeof(char));
      if (!result_array[j]) {
        printf("[Error]");
        return j;
      }
